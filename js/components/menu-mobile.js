...
class MenuMobileDMaior extends HTMLElement {
  ...
  render() {
    ...
    <style>
      ...
      *{ box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }
      ...
      .menu-content svg { color: var(--dm-effect-accent,var(--dm-rank-cyan,#00d4d4)); }
      ...
      .gear-btn.open svg{ transform:rotate(60deg); color:var(--dm-effect-accent,var(--dm-rank-cyan,#00d4d4)); }
      /* ── Sino de avisos ── */
      ...
      .bell-btn:hover{ color:var(--dm-effect-accent,var(--dm-rank-cyan,#00d4d4)); }
      ...
      @media (max-width:520px){
        ...
        .dd-option.active .dd-dot{ box-shadow:0 0 0 2.5px var(--dm-bg-2),0 0 0 4.5px var(--dm-effect-accent,var(--dm-rank-cyan,#00d4d4)); }
      ...
      .sidebar{ position:fixed; top:0; right:-100%; width:280px; max-width:85vw; height:100vh; background:var(--dm-grad-sidebar); border-left:1px solid var(--dm-effect-35,var(--dm-rank-cyan-35,rgba(0,212,212,.35))); box-shadow:-5px 0 30px var(--dm-shadow-lg),-2px 0 20px var(--dm-effect-glow,var(--dm-rank-glow,rgba(59,130,246,.28))); transition:right .3s cubic-bezier(.25,.8,.25,1); z-index:9999; display:flex; flex-direction:column; overflow-y:auto; scrollbar-width:none; }
      ...
      <div class="topbar-right">
        ...
        <div style="position:relative;">
          ...
          <div class="layout-dropdown" id="layoutDropdown">
            ...
            <div class="pref-row" style="grid-column:1/-1">
  ...
  bindEvents() {
    ...
    const fontFamilySelect = root.getElementById('ddFontFamily');
    if (window.DMaiorPrefs?.getFontFamily) {
      if (fontFamilySelect) fontFamilySelect.value = window.DMaiorPrefs.getFontFamily();
    }
    if (fontFamilySelect) {
      fontFamilySelect.addEventListener('change', () => {
        if (window.DMaiorPrefs) window.DMaiorPrefs.setFontFamily(fontFamilySelect.value);
      });
    }
  ...
  async checkUnread() {
    try {
      ...
      // Só "importante" vira notificação de verdade (ver loadAvisos em
      // dmaior-app.js) — sem esse filtro o sino acendia por causa de avisos
      // "rápido" que nunca aparecem na tela de Avisos, deixando o ponto
      // vermelho aceso pra sempre (o id nunca entra no set de "lidos").
