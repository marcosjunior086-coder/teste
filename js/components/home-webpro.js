...
class DmaiorHomeWebpro extends HTMLElement {
  ...
  _css() { return `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .ben{padding:26px 24px;}
    .ben .ic{display:flex;width:48px;height:48px;border-radius:14px;background:var(--dm-cyan-08);border:1px solid var(--dm-cyan-20);align-items:center;justify-content:center;margin-bottom:15px;}
    ...
    .ben-grid.reveal .ben:nth-child(even){transform:translateY(-32px);}
    .ben-grid.reveal .ben.in{opacity:1;transform:none;}
    @media (prefers-reduced-motion:reduce){ .ben-grid.reveal .ben{opacity:1!important;transform:none!important;} }
  ...
  _html() {
    ...
    <section class="wrap">
      <div class="surface strip cta-band">
        <div>
          <h2>Sua primeira live paga pode ser hoje.</h2>
          <p>Candidatura gratuita, resposta em até 24h e treinamento desde o primeiro dia.</p>
        </div>
        <div class="btns">
          <a href="recrutamento.html" class="btn-grad">Entrar na Agência <span class="tag">Gratuito</span></a>
          <a href="https://www.kwai.com/@dmaioragency" target="_blank" rel="noopener noreferrer" class="btn-outline">Ver nosso Kwai</a>
        </div>
      </div>
    </section>
    `;
  ...
  _bind() {
    ...
    // benefícios — revelação no scroll (só mobile)
