...
class DimaiorAdmin extends HTMLElement {
  ...
  async _carregarDiario(){
    ...
    const tabelaHtml=`<div class="rank-table-wrap"><table><thead><tr><th>#</th><th>Perfil</th><th>Nome</th><th>${this._ico('diamond',12)} Diamantes</th><th>Horas</th></tr></thead><tbody>${pagina.map(sv=>`<tr><td style="color:var(--t3);font-family:'Rajdhani',sans-serif;font-weight:700">${sv.posicao}</td><td>${this._avatar(this._proxyFoto(sv.foto||sv.foto_url||''),sv.nome||'')}</td><td><div style="font-family:'Rajdhani',sans-serif;font-weight:700;color:var(--t1)">${this._esc(sv.nome||'-')}</div><div style="font-size:10px;color:var(--cyan)">${this._esc(sv.kwai_uid||sv.kwai_id||'-')}</div></td><td style="color:var(--azul);font-family:'Rajdhani',sans-serif;font-weight:700">${this._num(this._diam(sv))}</td><td style="color:var(--t2)">${sv.horas||'-'}</td></tr>`).join('')}</tbody></table></div>`;
    const mobileHtml=`<div class="rank-mobile-only">${pagina.map(sv=>`<div class="rk-item"><div class="rk-preview"><span class="rk-pos">${sv.posicao}</span><div class="rk-av">${this._avatar(this._proxyFoto(sv.foto||sv.foto_url||''),sv.nome||'')}</div><div class="rk-info"><div class="rk-nome">${this._esc(sv.nome||'-')}</div><div class="rk-uid">${this._esc(sv.kwai_uid||sv.kwai_id||'-')}</div><div class="rk-diam">${this._ico('diamond',11)} ${this._num(this._diam(sv))}</div></div><div class="rk-right"><div class="rk-val">${sv.horas||'-'}</div></div></div></div>`).join('')}</div>`;
    const root=s.getElementById('root');
    const isMobile=root?.classList.contains('narrow')||window.matchMedia('(max-width:700px)').matches;
    el.innerHTML=(isMobile?mobileHtml:tabelaHtml)+pager;
      ...
      return`<div class="ph-acc-item">
        <div class="ph-acc-preview" onclick="this.closest('.ph-acc-item').classList.toggle('open')">
          <div style="display:flex;align-items:center;gap:8px;flex:1;min-width:0">
            <span style="font-family:'Rajdhani',sans-serif;font-size:14px;font-weight:700;color:var(--cyan)">${this._esc(p.mes_referencia)}</span>
  ...
  _safeImgSrc(u){if(!u||typeof u!=='string')return'';if(!/^https?:\/\//i.test(u.trim()))return'';return this._esc(u);}
  ...
  async _carregarCandidaturas(){
    ...
    el.innerHTML=`<table style="width:100%;border-collapse:collapse;font-size:12px">
      ...
      <tbody>${lista.map(c=>{
        ...
        const wppNum=String(c.whatsapp||'').replace(/\D/g,'');
