...
class DimaiorAdmin extends HTMLElement {
  ...
  _buildMobileNav(s){
    ...
    if(window.ResizeObserver){ new ResizeObserver(()=>this._updNotch()).observe(mnav); }
  ...
  async _buscarExterno(){
    ...
    prev.innerHTML=`<div style="display:flex;gap:12px;align-items:center">${this._avatar(foto,sv.nome,'av')}<div><div style="font-family:var(--dm-font-title,'Rajdhani',sans-serif);font-size:15px;font-weight:700;color:var(--t1)">${this._esc(sv.nome)}</div><div style="font-size:11px;color:var(--cyan)">UID: ${this._esc(sv.kwai_uid)}</div><div style="font-size:11px;color:var(--t3)">Kwai ID: ${this._esc(sv.kwai_id||'—')}</div>${d.ja_cadastrado?`<div style="font-size:10px;color:var(--warn);margin-top:4px">Já cadastrado nos externos</div>`:''}</div></div>`;
    acoes.style.display='flex';
    acoes.innerHTML=`<button id="mVerifExtSalvar" class="btn btn-o" style="border-color:rgba(0,212,212,.4);color:var(--cyan)">${this._ico('check_c',13)} Marcar Verificado</button><button id="mVerifExtPremium" class="btn btn-o" style="border-color:rgba(255,152,0,.5);color:var(--warn)">${this._ico('check_c',13)} Marcar Premium</button>`;
  ...
  async _buscarStreamerRanking(){
    ...
    const sv=d.streamer;const foto=this._proxyFoto(sv.foto||'');
    prev.style.display='block';
    prev.innerHTML=`<div style="display:flex;gap:12px;align-items:center">${this._avatar(foto,sv.nome,'av')}<div><div style="font-family:var(--dm-font-title,'Rajdhani',sans-serif);font-size:15px;font-weight:700;color:var(--t1)">${this._esc(sv.nome)}</div><div style="font-size:11px;color:var(--cyan)">UID: ${this._esc(sv.kwai_uid)}</div><div style="font-size:11px;color:var(--t3)">Kwai ID: ${this._esc(sv.kwai_id||'—')}</div>${d.ja_oculto?`<div style="font-size:10px;color:var(--warn);margin-top:4px">Já está oculto do ranking${d.motivo?' — '+this._esc(d.motivo):''}</div>`:''}</div></div>`;
    if(d.ja_oculto){
      acoes.style.display='flex';
  ...
  async _carregarConfig(){
    ...
    d.config.filter(c=>labels[c.chave]?.bool).forEach(c=>{
      s.getElementById(`cfg_${c.chave}`)?.addEventListener('change',async e=>{
        ...
        else{this._toast(r?.erro||'Erro','err');e.target.checked=!e.target.checked;}
      });
    });
    d.config.filter(c=>!labels[c.chave]?.readonly&&!labels[c.chave]?.bool).forEach(c=>{
  ...
  async _carregarUids(){
    ...
    const busca=s.getElementById('uidBusca')?.value.trim()||'';
    const d=await this._api('GET',`/admin/uids?pagina=${this._pg.uid}&status=${filtro}${busca?`&busca=${encodeURIComponent(busca)}`:''}`);
    ...
    if(container)container.innerHTML=lista.map(u=>{const contaCriada=u.ativo&&u.utilizado;const sT=!u.ativo?'Revogado':u.bloqueado?'Bloqueado':u.utilizado?'Conta Criada':'Aguardando';const sC=!u.ativo?'off':u.bloqueado?'off':u.utilizado?'on neutro':'on';const nomeExibir=(u.nome_streamer&&u.nome_streamer!==u.kwai_uid)?u.nome_streamer:(u.nome_ref||'');return`<div class="uid-row"><div class="uid-row-main">${this._avatar(u.foto_streamer,nomeExibir||u.kwai_uid,'av')}<div style="min-width:0;flex:1"><div class="uid-kwai">${this._ico('key_uid',14)} ${this._esc(u.kwai_uid)}</div>${nomeExibir?`<div class="uid-nome-ref">${this._esc(nomeExibir)}</div>`:''}</div><span class="badge ${sC}">${sT}</span></div><div class="uid-row-meta"><span>${this._ico('users',11)} por <strong>${this._esc(u.autorizado_por||'admin')}</strong></span><span>${this._ico('calendar',11)} ${this._fdtCurto(u.autorizado_em)}</span>${u.utilizado_em?`<span>${this._ico('check_c',11)} usado ${this._fdtCurto(u.utilizado_em)}</span>`:''}</div>${u.ativo&&!u.utilizado?`<button class="btn btn-o btn-sm btn-d uid-revogar" data-uid="${this._esc(u.kwai_uid)}">${this._ico('x_circle',11)} Revogar</button>`:''}${!u.ativo?`<button class="btn btn-o btn-sm uid-reativar" data-uid="${this._esc(u.kwai_uid)}" style="border-color:rgba(74,222,128,.4);color:var(--verde)">${this._ico('check_c',11)} Reativar</button>`:''}${contaCriada?`<div class="uid-row-acoes" style="display:flex;gap:6px;margin-top:6px">${u.bloqueado?`<button class="btn btn-o btn-sm uid-desbloquear" data-uid="${this._esc(u.kwai_uid)}" style="border-color:rgba(74,222,128,.4);color:var(--verde)">${this._ico('unlock',11)} Desbloquear</button>`:`<button class="btn btn-o btn-sm uid-bloquear" data-uid="${this._esc(u.kwai_uid)}">${this._ico('lock_r',11)} Bloquear Acesso</button>`}<button class="btn btn-o btn-sm uid-resetar-senha" data-uid="${this._esc(u.kwai_uid)}">${this._ico('key_uid',11)} Senha Temporária</button><button class="btn btn-o btn-sm btn-d uid-apagar-conta" data-uid="${this._esc(u.kwai_uid)}">${this._ico('x_circle',11)} Apagar Conta</button></div>`:''}</div>`;}).join('');
    this._renderPg('pgUID',this._pg.uid,lista.length,30,n=>{this._pg.uid=n;this._carregarUids();});
    if(container){container.querySelectorAll('.uid-revogar').forEach(btn=>{btn.addEventListener('click',()=>this._revogarUid(btn.dataset.uid,false));});container.querySelectorAll('.uid-reativar').forEach(btn=>{btn.addEventListener('click',()=>this._revogarUid(btn.dataset.uid,true));});container.querySelectorAll('.uid-bloquear').forEach(btn=>{btn.addEventListener('click',()=>this._bloquearAcessoUid(btn.dataset.uid,true));});container.querySelectorAll('.uid-desbloquear').forEach(btn=>{btn.addEventListener('click',()=>this._bloquearAcessoUid(btn.dataset.uid,false));});container.querySelectorAll('.uid-resetar-senha').forEach(btn=>{btn.addEventListener('click',()=>this._resetarSenhaUid(btn.dataset.uid));});container.querySelectorAll('.uid-apagar-conta').forEach(btn=>{btn.addEventListener('click',()=>this._apagarContaUid(btn.dataset.uid));});}
  }
  async _bloquearAcessoUid(uid,bloquear=true){
    const acao=bloquear?'bloquear o acesso':'desbloquear o acesso';if(!confirm(`Confirmar ${acao} do UID ${uid}?\n\nA conta, carteira e histórico continuam intactos — só o login é ${bloquear?'suspenso':'liberado'}.`))return;
    const d=await this._api('POST',`/admin/uids/${uid}/${bloquear?'bloquear':'desbloquear'}`);
    if(d?.ok){this._toast(`Acesso ${bloquear?'bloqueado':'desbloqueado'}!`);this._carregarUids();}else this._toast(d?.erro||'Erro','err');
  }
  async _apagarContaUid(uid){
  ...
  async _resetarSenhaUid(uid){
    if(!confirm(`Gerar uma senha temporária para o UID ${uid}?\n\nA senha atual dele para de funcionar. Ele deve trocá-la assim que entrar (aba Perfil do painel).`))return;
    const d=await this._api('POST',`/admin/uids/${uid}/resetar-senha`);
    if(d?.ok){
      navigator.clipboard?.writeText(d.senha_temporaria).catch(()=>{});
      alert(`Senha temporária gerada para o UID ${uid}:\n\n${d.senha_temporaria}\n\n(já copiada para a área de transferência)\n\nPasse essa senha para o streamer. Ele deve trocá-la em Perfil assim que entrar no painel.`);
      this._toast('Senha temporária gerada e copiada!');
    }else this._toast(d?.erro||'Erro','err');
  }
        ...
        return`<div class="sc-item">
          ...
          <div class="sc-body">
            <div class="sc-body-grid">
              <div class="sc-detalhe"><div class="sc-dlbl">Pendente</div><div class="sc-dval" style="color:var(--amber)">${c.saldo_pendente>0?this._brl(c.saldo_pendente):'—'}</div></div>
  ...
  async _pagarSaqueMp(id,nome,valor,pixInfo){
    ...
    const d=await this._api('POST',`/admin/saques/${id}/pagar`,{});btns.forEach(b=>{b.disabled=false;});if(btnPagar)btnPagar.innerHTML=`<img src="assets/icons/pix.webp" style="width:13px;height:13px;object-fit:contain;flex-shrink:0" alt="pix"> Pagar PIX`;
  ...
  _abrirModalProcessar(){
    ...
    const taxa=this._taxaSaque||0,taxaP=this._taxaPerc||0;const taxaInfo=s.getElementById('mProcTaxaInfo');if(taxaInfo){if(taxa>0||taxaP>0){taxaInfo.innerHTML=`<div style="padding:8px 12px;background:rgba(251,191,36,.08);border:1px solid rgba(251,191,36,.25);border-radius:var(--rs);font-size:11px;color:var(--amber);margin-bottom:10px">${this._ico('warning',12)} Taxa: ${taxa>0?this._brl(taxa)+' fixo':''}${taxaP>0?' + '+taxaP+'%':''}</div>`;}else{taxaInfo.innerHTML=`<div style="padding:6px 12px;background:rgba(74,222,128,.05);border:1px solid rgba(74,222,128,.15);border-radius:var(--rs);font-size:10px;color:var(--t3);margin-bottom:10px">${this._ico('check_c',11)} Sem taxa.</div>`;}}
  ...
  _bindEvents(){
    ...
    // v2: UIDs
    s.getElementById('btnAtuUIDs').addEventListener('click',()=>this._carregarUids());s.getElementById('btnNovoUID').addEventListener('click',()=>this._abrirModalUID());s.getElementById('btnBuscarUID').addEventListener('click',()=>this._executarBuscaUID());s.getElementById('uidInputVal').addEventListener('keydown',e=>{if(e.key==='Enter')this._executarBuscaUID();});s.getElementById('btnConfirmarUID').addEventListener('click',()=>this._confirmarAutorizarUID());s.getElementById('btnCancelarUID').addEventListener('click',()=>this._fechaModal('mUID'));s.getElementById('uidFiltro').addEventListener('change',()=>{this._pg.uid=1;this._carregarUids();});s.getElementById('uidBusca').addEventListener('input',this._dbc(()=>{this._pg.uid=1;this._carregarUids();},400));
    // v2: Carteira
    ...
    s.getElementById('pkRankingModo').addEventListener('change',()=>this._pkRankingModoMudou());
    s.getElementById('pkRankingPeriodoBtn').addEventListener('click',()=>this._abrirSeletorPeriodo('pkRankingPeriodoBtn','pkRankingDataDe','pkRankingDataAte','pkRankingPeriodoTexto'));
    s.getElementById('btnAtualizarRankingPk').addEventListener('click',()=>this._carregarRankingPk());
    // Monitor Kwai
  ...
  // ── PK DIÁRIO ───────────────────────────────────────────────
  async _carregarPkDiario(){
    this._pkAbertaId=null;
    this.shadowRoot.getElementById('pkListaWrap').style.display='block';
    this.shadowRoot.getElementById('pkFormWrap').style.display='none';
    this.shadowRoot.getElementById('pkDetalheWrap').style.display='none';
  ...
  _abrirFormPk(){
    ...
    s.getElementById('pkFormWrap').style.display='block';
  }
  ...
  _pkCriterioValor(){
    const s=this.shadowRoot,sel=s.getElementById('pkCriterioSelect').value;
    if(sel==='0')return 0;
  ...
  async _buscarElegiveisPk(){
    ...
    this._pkElegiveis.forEach(st=>{
      if(st.elegivel && !this._pkSelecionados.has(st.kwai_uid)){
        this._pkSelecionados.set(st.kwai_uid,{...st,origem:'elegivel'});
      }
    });
    this._renderPkElegiveis();
  }
  ...
  _renderPkElegiveis(){
    ...
    if(!this._pkElegiveis.length && !this._pkSelecionados.size){
      ...
      this._pkSelecionados.forEach((st,uid)=>{ if(!porUid.has(uid)) porUid.set(uid,st); });
      const linhas=[...porUid.values()].sort((a,b)=>(b.diamantes||0)-(a.diamantes||0));
      ...
      area.querySelectorAll('.pk-elig-chk').forEach(chk=>chk.addEventListener('change',()=>{
        ...
        if(chk.checked)this._pkSelecionados.set(uid,{...st,origem:st.origem||'elegivel'});
        else this._pkSelecionados.delete(uid);
        this._atualizarContadorPk();
      }));
    }
    this._atualizarContadorPk();
  }

  _atualizarContadorPk(){
    const el=this.shadowRoot.getElementById('pkParticipantesCount');
    if(el)el.textContent=`${this._pkSelecionados.size} selecionado${this._pkSelecionados.size===1?'':'s'}`;
  }

  _pkSelecionarTodosElegiveis(){
    this._pkElegiveis.filter(st=>st.elegivel).forEach(st=>this._pkSelecionados.set(st.kwai_uid,{...st,origem:'elegivel'}));
    this._renderPkElegiveis();
  }

  async _pkAddManual(){
    const s=this.shadowRoot,inp=s.getElementById('pkManualUid'),uid=inp.value.trim();
    if(!uid)return;
    if(this._pkSelecionados.has(uid)){this._toast('Esse UID já está na lista','err');return;}
    const d=await this._api('GET',`/admin/streamers?uid=${encodeURIComponent(uid)}`);
    const perfil=(d?.perfis||[])[0];
    this._pkSelecionados.set(uid,{
      kwai_uid:uid, nome:perfil?.nome||uid, foto:perfil?.foto||null,
      diamantes:0, novato:true, origem:'manual',
    });
    inp.value='';
    this._renderPkElegiveis();
  }
  ...
  _pkResultadoLabel(r){
    return {pendente:'Pendente',vitoria_a:'Vitória A',vitoria_b:'Vitória B',empate:'Empate',cancelado:'Cancelado'}[r]||r;
  }

  _renderPkConfrontos(lista){
    ...
    el.querySelectorAll('[data-pk-conf-del]').forEach(b=>b.addEventListener('click',()=>{
      this._confirmarDel('Excluir este confronto?',()=>this._excluirConfrontoPk(b.dataset.pkConfDel));
    }));
  }

  async _lancarResultadoPk(id,resultado){
    const r=await this._api('PUT',`/admin/pk/confrontos/${id}`,{resultado});
    if(r?.ok){this._toast('Resultado atualizado!');this._abrirDetalhePk(this._pkAbertaId);}
    else{this._toast(r?.erro||'Erro ao lançar resultado','err');this._abrirDetalhePk(this._pkAbertaId);}
  }

  async _excluirConfrontoPk(id){
    const r=await this._api('DELETE',`/admin/pk/confrontos/${id}`);
    if(r?.ok){this._toast('Confronto excluído');this._abrirDetalhePk(this._pkAbertaId);}
    else this._toast(r?.erro||'Erro ao excluir','err');
  }

  async _pkCriarConfrontoManual(){
    const s=this.shadowRoot;
  ...
  async _abrirRankingPk(){
    const s=this.shadowRoot;
    s.getElementById('pkListaWrap').style.display='none';
    s.getElementById('pkFormWrap').style.display='none';
    s.getElementById('pkDetalheWrap').style.display='none';
    s.getElementById('pkRankingWrap').style.display='block';

    const sel=s.getElementById('pkRankingModo');
    if(!sel.dataset.carregado){
      const d=await this._api('GET','/admin/pk/programacoes');
      (d?.programacoes||[]).forEach(p=>{
        const o=document.createElement('option');
        o.value=`prog:${p.id}`; o.textContent=p.nome;
        sel.appendChild(o);
      });
      sel.dataset.carregado='1';
    }
    // padrão: mês atual
    const hoje=new Date(),ini=new Date(hoje.getFullYear(),hoje.getMonth(),1),fim=new Date(hoje.getFullYear(),hoje.getMonth()+1,0);
    const iso=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
    if(!s.getElementById('pkRankingDataDe').value){
      s.getElementById('pkRankingDataDe').value=iso(ini);
      s.getElementById('pkRankingDataAte').value=iso(fim);
      s.getElementById('pkRankingPeriodoTexto').textContent=`${ini.toLocaleDateString('pt-BR')} até ${fim.toLocaleDateString('pt-BR')}`;
    }
    this._carregarRankingPk();
  }

  _pkRankingModoMudou(){
    const s=this.shadowRoot,ehPeriodo=s.getElementById('pkRankingModo').value==='periodo';
    s.getElementById('pkRankingPeriodoWrap').style.display=ehPeriodo?'block':'none';
  }

  async _carregarRankingPk(){
    const s=this.shadowRoot,area=s.getElementById('pkRankingArea');
    const modo=s.getElementById('pkRankingModo').value;
    let query;
    if(modo==='periodo'){
      const de=s.getElementById('pkRankingDataDe').value,ate=s.getElementById('pkRankingDataAte').value;
      if(!de||!ate){this._toast('Selecione um período','err');return;}
      query=`data_inicio=${de}&data_fim=${ate}`;
    } else {
      query=`programacao_id=${modo.replace('prog:','')}`;
    }
    area.innerHTML=this._loading();
    ...
    const tabelaHtml=`<div class="rank-table-wrap"><table style="width:100%;border-collapse:collapse;font-size:12px">
      ...
      <tbody>
        ${pagina.map(r=>`<tr style="border-top:1px solid var(--brddim)">
          ...
          <td style="text-align:center;color:var(--verde)">${r.vitorias}</td>
          <td style="text-align:center;color:var(--verm)">${r.derrotas}</td>
          <td style="text-align:center;color:var(--t3)">${r.empates}</td>
          <td style="text-align:center;color:var(--t3)">${r.pendentes}</td>
          <td style="text-align:center">${r.aproveitamento!=null?r.aproveitamento+'%':'—'}</td>
  ...
  _css(){return`
    :host{display:block;width:100%;height:auto;min-height:600px;overflow:visible;background:#060B16;
      ...
      /* semânticos de status/badge — constantes entre temas de propósito */
      --warn:#ff9800;--amber:#fbbf24;--info:#818cf8;--orange:#fb923c;--purple:#a855f7;
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .uid-lookup-card{display:flex;gap:12px;padding:14px;border-radius:var(--rs);margin-bottom:8px}.uid-lookup-card.ok{background:rgba(74,222,128,.06);border:1px solid rgba(74,222,128,.3)}.uid-lookup-warn{display:flex;align-items:flex-start;gap:8px;padding:12px 14px;border-radius:var(--rs);background:rgba(251,191,36,.08);border:1px solid rgba(251,191,36,.3);color:var(--amber);font-size:12px;line-height:1.5;margin-bottom:8px}.uid-lookup-err{display:flex;align-items:flex-start;gap:8px;padding:12px 14px;border-radius:var(--rs);background:rgba(248,113,113,.08);border:1px solid rgba(248,113,113,.3);color:var(--verm);font-size:12px;margin-bottom:8px}
    ...
    @media(max-width:700px){
      ...
      .mnav .grp{display:flex;gap:32px;position:relative;z-index:1;}
      .mnav button{display:grid;place-items:center;background:none;border:none;color:var(--t3);padding:8px;cursor:pointer;}
        ...
        <div class="shell">
          ...
          <div class="content">
            ...
            <div class="pag" id="pag-pkDiario">${ph('PK Diário','zap','Liga contínua entre streamers — fecha o dia e já monta os pares seguintes sozinha','btnAtuPk',`<button class="btn btn-o" id="btnVerRankingPk">${this._ico('trophy',13)} Ranking</button><button class="btn btn-g" id="btnNovaPk">${this._ico('plus',13)} Nova Liga</button>`)}
              ...
              <div class="box" id="pkFormWrap" style="display:none">
                ...
                <div style="padding:16px">
                  ...
                  <div class="cfg-row" style="flex-wrap:wrap">
                    ...
                    <label id="pkCriterioCustomWrap" style="display:none"><div class="cfg-chave">Valor</div><input class="cfg-inp" type="number" id="pkCriterioCustom" style="width:100px"></label>
              ...
              <div class="box" id="pkRankingWrap" style="display:none">
                <div class="bhead"><div class="btitulo">${this._ico('trophy',14)} Ranking de Vitórias</div>
                  <div class="bacoes"><button class="btn btn-o btn-sm" id="btnVoltarRankingPk">${this._ico('arrow_right',12)} Voltar</button></div>
                </div>
                <div style="padding:16px">
                  <div class="cfg-row" style="flex-wrap:wrap">
                    <label><div class="cfg-chave">Ver por</div>
                      <select class="cfg-inp" id="pkRankingModo" style="width:210px">
                        <option value="periodo">Período personalizado</option>
                      </select>
                    </label>
                    <label id="pkRankingPeriodoWrap"><div class="cfg-chave">Período</div>
                      <button type="button" class="periodo-campo" id="pkRankingPeriodoBtn">${this._ico('calendar',13)} <span id="pkRankingPeriodoTexto">Selecionar período</span></button>
                      <input type="hidden" id="pkRankingDataDe"><input type="hidden" id="pkRankingDataAte">
                    </label>
                    <button class="btn btn-o btn-sm" id="btnAtualizarRankingPk" style="margin-top:16px">${this._ico('refresh',12)} Atualizar</button>
                  </div>
                  <div id="pkRankingArea" style="overflow-x:auto;margin-top:8px">${this._empty('trophy','Escolha um período ou programação e clique em Atualizar')}</div>
                </div>
              </div>
            </div>
  ...
  _solicStatusInfo(st){
    const m={
      pendente_analise:['var(--amber)','Pendente de análise'],
      aprovada:['#4ade80','Aprovada'],
      aguardando_envio:['#00d4d4','Aguardando envio ao Google Forms'],
      enviada:['var(--info)','Enviada'],
      recusada:['#f87171','Recusada'],
      necessita_correcao:['var(--orange)','Necessita correção'],
    };
  ...
  async _abrirDetalheSolicForm(id){
    ...
    over.innerHTML=`<div style="background:var(--card,#0d1424);border:1px solid var(--brd);border-radius:14px;max-width:640px;width:100%;position:relative;font-family:var(--dm-font-body,'Exo 2',sans-serif)">
      ...
      <div style="padding:18px 20px">
        ...
        ${s.motivo_correcao?`<div style="font-size:12px;color:var(--orange);margin-bottom:8px">Correção pedida: ${this._esc(s.motivo_correcao)}</div>`:''}
        ...
        ${s.enviado_em?`<div style="font-size:12px;color:var(--info);margin-bottom:8px">Enviada em ${this._fdt(s.enviado_em)} por ${this._esc(s.enviado_por||'—')}${s.envio_obs?` · ${this._esc(s.envio_obs)}`:''}${s.envio_comprovante_url?` · <a href="${this._esc(s.envio_comprovante_url)}" target="_blank" rel="noopener" style="color:var(--cyan)">comprovante</a>`:''}</div>`:''}
        ...
        <div id="dsfAcoes" style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          ${podeEnviar?`<button class="btn btn-sm btn-o" id="dsfEnviar" style="border-color:rgba(129,140,248,.5);color:var(--info)">${this._ico('check',12)} Marcar como enviado</button>`:''}
          ${podeCorrigir?`<button class="btn btn-sm" id="dsfCorrecao" style="border:1px solid rgba(251,146,60,.4);color:var(--orange);background:rgba(251,146,60,.08)">${this._ico('edit',12)} Solicitar correção</button>`:''}
    ...
    over.querySelector('#dsfCorrecao')?.addEventListener('click',()=>{
      painel.innerHTML=`<div style="padding:12px;border:1px solid var(--brddim);border-radius:8px">
        ...
        <button class="btn btn-sm" id="dsfCorrecaoOk" style="margin-top:8px;border:1px solid rgba(251,146,60,.4);color:var(--orange);background:rgba(251,146,60,.08)">Enviar pedido de correção</button>
      </div>`;
    ...
    over.querySelector('#dsfEnviar')?.addEventListener('click',()=>{
      ...
      painel.innerHTML=`<div style="padding:12px;border:1px solid var(--brddim);border-radius:8px;display:flex;flex-direction:column;gap:8px">
        ...
        <button class="btn btn-sm btn-o" id="dsfEnviarOk" style="border-color:rgba(129,140,248,.5);color:var(--info)">Confirmar envio</button>
      </div>`;
  ...
  async _carregarConfigForm(){
    ...
    el.innerHTML=`<table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="border-bottom:1px solid var(--brddim)">
      ...
      const rasc=f.tem_rascunho?' <span style="font-size:10px;color:var(--orange)">• rascunho</span>':'';
      return `<tr style="border-bottom:1px solid var(--brddim)">
