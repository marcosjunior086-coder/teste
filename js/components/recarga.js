...
class DmaiorRecarga extends HTMLElement {
  ...
  #render() {
    this.innerHTML = `
      <style>
        ...
        :host, dmaior-recarga { display:block; width:100%; }
        .dm-wrap * { box-sizing:border-box; margin:0; padding:0; }

        .dm-wrap {
          font-family:'Exo 2',sans-serif; background:#080c14;
          border-radius:20px; width:100%; max-width:420px; height:640px;
          display:flex; flex-direction:column; overflow:hidden; position:relative;
          box-shadow: 0 0 0 1px rgba(0,242,255,0.12), 0 25px 60px rgba(0,0,0,0.7), 0 0 40px rgba(0,242,255,0.06);
        }
        .dm-wrap::before {
          content:''; position:absolute; inset:0;
          background:radial-gradient(ellipse 80% 60% at 50% -10%,rgba(0,242,255,0.08) 0%,transparent 70%);
          pointer-events:none; z-index:0;
        }

        .dm-header {
          position:relative; z-index:1; padding:16px 20px;
          background:rgba(0,242,255,0.04); border-bottom:1px solid rgba(0,242,255,0.12);
        ...
        .dm-logo-icon {
          ...
          background:linear-gradient(135deg,#00f2ff22,#00f2ff44);
          border:1px solid rgba(0,242,255,0.35); border-radius:10px;
          display:flex; align-items:center; justify-content:center;
          box-shadow:0 0 12px rgba(0,242,255,0.25);
        }
        .dm-brand-text { display:flex; flex-direction:column; }
        .dm-brand-name { font-family:'Rajdhani',sans-serif; font-size:15px; font-weight:700; letter-spacing:2px; color:#fff; text-transform:uppercase; }
        .dm-brand-sub  { font-size:10px; color:rgba(0,242,255,0.6); letter-spacing:1.5px; text-transform:uppercase; }
        .dm-status     { display:flex; align-items:center; gap:6px; font-size:11px; color:rgba(0,242,255,0.7); }
        .dm-dot        { width:7px; height:7px; border-radius:50%; background:#00f2ff; box-shadow:0 0 6px #00f2ff; animation:dm-pulse 2s infinite; }
        ...
        .dm-body::-webkit-scrollbar-thumb { background:rgba(0,242,255,0.2); border-radius:4px; }
        ...
        .dm-msg.bot  { background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08); color:#dde6f0; align-self:flex-start; border-bottom-left-radius:4px; }
        .dm-msg.bot strong { color:#00f2ff; }
        .dm-msg.user { background:linear-gradient(135deg,#00b4d8,#00f2ff); color:#000; font-weight:600; align-self:flex-end; border-bottom-right-radius:4px; box-shadow:0 0 14px rgba(0,242,255,0.35); }
        .dm-msg.erro { background:rgba(255,60,60,0.12); border:1px solid rgba(255,80,80,0.3); color:#ff9090; align-self:flex-start; }

        .dm-card-perfil { background:rgba(0,242,255,0.06); border:1px solid rgba(0,242,255,0.2); border-radius:14px; padding:14px; display:flex; align-items:center; gap:13px; align-self:flex-start; max-width:88%; animation:dm-fadein .35s ease; }
        .dm-card-perfil img { width:52px; height:52px; border-radius:50%; border:2px solid rgba(0,242,255,0.4); object-fit:cover; }
        .dm-perfil-nome { font-family:'Rajdhani',sans-serif; font-size:15px; font-weight:700; color:#fff; margin-bottom:2px; }
        .dm-perfil-id   { font-size:11px; color:rgba(0,242,255,0.6); margin-bottom:4px; }
        .dm-perfil-link { font-size:11px; color:#00f2ff; text-decoration:none; }
        ...
        .dm-btn.primary   { background:linear-gradient(135deg,#00b4d8,#00f2ff); color:#000; box-shadow:0 0 14px rgba(0,242,255,0.3); }
        .dm-btn.primary:hover { transform:scale(1.04); filter:brightness(1.1); }
        .dm-btn.secondary { background:rgba(255,255,255,0.06); color:#aac; border:1px solid rgba(255,255,255,0.12); }
        ...
        .dm-calc { background:rgba(0,242,255,0.05); border:1px solid rgba(0,242,255,0.15); border-radius:14px; padding:14px; }
        .dm-calc-label { font-size:11px; color:rgba(0,242,255,0.6); letter-spacing:1px; text-transform:uppercase; margin-bottom:8px; }
        .dm-calc-input-row { display:flex; align-items:center; gap:8px; margin-bottom:10px; }
        .dm-calc-input { flex:1; background:rgba(0,0,0,0.35); border:1px solid rgba(0,242,255,0.25); border-radius:8px; padding:9px 12px; color:#fff; font-size:16px; font-weight:600; font-family:'Rajdhani',sans-serif; outline:none; transition:.25s; -moz-appearance:textfield; }
        .dm-calc-input::-webkit-outer-spin-button, .dm-calc-input::-webkit-inner-spin-button { -webkit-appearance:none; }
        .dm-calc-input:focus { border-color:#00f2ff; box-shadow:0 0 8px rgba(0,242,255,0.25); }
        .dm-calc-unit { font-size:11px; color:rgba(0,242,255,0.5); white-space:nowrap; }
        ...
        .dm-calc-label-sm { font-size:12px; color:#889; }
        .dm-calc-valor { font-family:'Rajdhani',sans-serif; font-size:20px; font-weight:700; color:#00f2ff; transition:.2s; }
        ...
        .dm-form { background:rgba(0,242,255,0.05); border:1px solid rgba(0,242,255,0.15); border-radius:14px; padding:14px; display:flex; flex-direction:column; gap:10px; }
        .dm-form-titulo { font-family:'Rajdhani',sans-serif; font-size:13px; font-weight:600; color:rgba(0,242,255,0.7); letter-spacing:.8px; text-transform:uppercase; border-bottom:1px solid rgba(0,242,255,0.1); padding-bottom:8px; }
        .dm-form-field { display:flex; flex-direction:column; gap:4px; }
        .dm-form-field label { font-size:11px; color:rgba(0,242,255,0.6); letter-spacing:1px; text-transform:uppercase; }
        .dm-form-field input { background:rgba(0,0,0,0.35); border:1px solid rgba(0,242,255,0.2); border-radius:8px; padding:9px 12px; color:#fff; font-size:14px; font-family:'Exo 2',sans-serif; outline:none; transition:.25s; }
        .dm-form-field input:focus { border-color:#00f2ff; box-shadow:0 0 8px rgba(0,242,255,0.2); }
        ...
        .dm-pix-frame-titulo { font-family:'Rajdhani',sans-serif; font-size:13px; font-weight:700; color:#00f2ff; letter-spacing:.8px; text-transform:uppercase; display:flex; align-items:center; gap:6px; }
        .dm-pix-frame-timer  { font-family:'Rajdhani',sans-serif; font-size:12px; font-weight:600; color:#ffc800; }
        .dm-pix-iframe { width:100%; height:420px; border:1px solid rgba(0,242,255,0.15); border-top:none; border-radius:0 0 14px 14px; background:#fff; display:block; }
        .dm-pix-fallback { background:rgba(0,0,0,0.3); border:1px solid rgba(0,242,255,0.15); border-top:none; border-radius:0 0 14px 14px; padding:16px; text-align:center; display:none; flex-direction:column; gap:10px; }
        .dm-pix-fallback p { font-size:12px; color:#889; line-height:1.5; }
        .dm-pagar-ext-btn { background:linear-gradient(135deg,#00b4d8,#00f2ff); color:#000; font-family:'Rajdhani',sans-serif; font-size:15px; font-weight:700; letter-spacing:1px; border:none; border-radius:10px; padding:12px; cursor:pointer; text-transform:uppercase; width:100%; transition:all .25s; box-shadow:0 4px 18px rgba(0,242,255,0.3); -webkit-tap-highlight-color:transparent; }
        .dm-pagar-ext-btn:hover { transform:scale(1.02); filter:brightness(1.1); }

        .dm-typing { display:flex; align-items:center; gap:5px; padding:10px 14px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.07); border-radius:14px; border-bottom-left-radius:4px; align-self:flex-start; animation:dm-fadein .2s ease; }
        .dm-typing span { width:6px; height:6px; border-radius:50%; background:rgba(0,242,255,0.6); animation:dm-blink 1.2s infinite; }
        ...
        .dm-input-area { position:relative; z-index:1; padding:12px 14px; background:rgba(0,0,0,0.4); border-top:1px solid rgba(0,242,255,0.08); display:flex; gap:8px; align-items:center; flex-shrink:0; }
        .dm-input { flex:1; background:rgba(255,255,255,0.05); border:1px solid rgba(0,242,255,0.15); border-radius:10px; padding:11px 14px; color:#fff; font-size:14px; font-family:'Exo 2',sans-serif; outline:none; transition:.25s; height:42px; }
        .dm-input:focus { border-color:rgba(0,242,255,0.5); box-shadow:0 0 8px rgba(0,242,255,0.15); }
        .dm-input:disabled { opacity:.4; cursor:not-allowed; }
        .dm-input::placeholder { color:#445; }
        .dm-send { width:42px; height:42px; background:linear-gradient(135deg,#00b4d8,#00f2ff); border:none; border-radius:10px; cursor:pointer; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:all .25s; box-shadow:0 0 12px rgba(0,242,255,0.25); }
        ...
        .dm-ajuda-btn { display:flex; align-items:center; gap:5px; background:none; border:none; padding:0 2px; color:rgba(0,242,255,0.45); font-size:11px; font-family:'Exo 2',sans-serif; cursor:pointer; transition:.2s; width:fit-content; -webkit-tap-highlight-color:transparent; }
        .dm-ajuda-btn:hover { color:rgba(0,242,255,0.85); }
        ...
        .dm-ajuda-inner { background:#0d1520; border:1px solid rgba(0,242,255,0.2); border-radius:16px; overflow:hidden; width:100%; max-width:340px; box-shadow:0 20px 60px rgba(0,0,0,0.8); }
        .dm-ajuda-header { display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(0,242,255,0.1); font-family:'Rajdhani',sans-serif; font-size:14px; font-weight:600; color:#fff; }
        .dm-ajuda-fechar { background:rgba(255,255,255,0.07); border:1px solid rgba(255,255,255,0.1); border-radius:6px; width:28px; height:28px; color:#aaa; cursor:pointer; font-size:13px; display:flex; align-items:center; justify-content:center; transition:.2s; }
        .dm-ajuda-fechar:hover { background:rgba(255,60,60,0.15); color:#ff8888; }
        .dm-ajuda-img { width:100%; display:block; max-height:320px; object-fit:contain; background:#080c14; }
  ...
  #mostrarIframeEnjoy() {
    ...
    wrap.querySelector('#dm-fallback-btn')?.addEventListener('click', () => window.open(this.#epUrl,'_blank'));
    body.appendChild(wrap);
  ...
  #mostrarBotaoPagarExterno(url) {
    ...
    card.querySelector('button').addEventListener('click', () => window.open(url,'_blank'));
    body.appendChild(card);
    this.#statusCard = document.createElement('div');
    this.#statusCard.className = 'dm-status-card aguardando';
