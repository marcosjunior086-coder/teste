class RankingDmaior extends HTMLElement {
    ...
    _getToken() {
        return localStorage.getItem('dmaior_token') || this._sessionToken || '';
    }
    ...
    _isLoggedIn() {
        return !!this._getToken() || localStorage.getItem('agencia_auth') === 'true';
    }
    ...
    async doLogin() {
        ...
        try {
            const res  = await fetch(`${this.CLOUDFLARE_API}/login`, {
                method: 'POST',
            ...
            if (data.sucesso && data.token) {
                if (this.shadowRoot.getElementById('remember').checked) {
                    localStorage.setItem('dmaior_token', data.token);
                } else {
    ...
    async fetchLives() {
        ...
        try {
            const res = await fetch(`${this.CLOUDFLARE_API}/api/lives`, { headers: { ...this._authHeader() } });
            if (!res.ok) return;
            const data = await res.json();
    ...
    async loadTabData() {
        ...
        try {
            if (config.tipo === 'supabase_mes' || config.tipo === 'supabase_diario') {
                await this.fetchPrizes();
                const endpoint = config.tipo === 'supabase_mes'
                    ? `${this.CLOUDFLARE_API}/api/ranking/v2`
                    : `${this.CLOUDFLARE_API}/api/ranking/diario`;
                const rows = await this.fetchSupabaseRanking(endpoint);
                this.allRows  = rows;
                this.prevRows = [];
                const hoje = new Date();
                ...
                const label = config.tipo === 'supabase_mes'
                    ? `01/${mm}/${ano} - ${ult}/${mm}/${ano}`
                    : `DIA ${String(hoje.getDate()).padStart(2, '0')}/${mm}`;
                this.shadowRoot.getElementById('info-date').textContent = label;
                ...
                const timerStr  = `${String(ultimoDia.getDate()).padStart(2,'0')}/${String(ultimoDia.getMonth()+1).padStart(2,'0')}/${ultimoDia.getFullYear()}`;
                this.startTimer(timerStr);
    ...
    async fetchSupabaseRanking(endpoint) {
        const res  = await fetch(endpoint, { headers: { ...this._authHeader() } });
        ...
        if (_safeP) this.VERIFICADO_PREMIUM_SVG = `<img class="verified-badge premium" src="${_safeP.replace(/"/g,'&quot;')}" width="15" height="15" title="Verificado Premium">`;
    ...
    // ── v3: fetchPrizes — lê JSON do Supabase via worker ─────────────────────
    // Worker retorna: { ok: true, premiosD: [{posicao, valor}, ...], premiosH: [...] }
    // Formatação em pt-BR feita aqui no browser (toLocaleString funciona no browser)
    async fetchPrizes() {
        try {
            const res  = await fetch(`${this.CLOUDFLARE_API}/premios`, { headers: { ...this._authHeader() } });
            if (!res.ok) return;
            const data = await res.json();
            if (!data?.ok) return;
            ...
            // posicao 1 → índice 0, posicao 2 → índice 1, etc.
            (data.premiosD || []).forEach(p => {
                if (p.valor > 0) this.prizesD[p.posicao - 1] = fmt(p.valor);
            });
            (data.premiosH || []).forEach(p => {
                if (p.valor > 0) this.prizesH[p.posicao - 1] = fmt(p.valor);
            });
        } catch (e) {
            console.error('fetchPrizes erro:', e);
        }
    }
    ...
    renderScreen() {
        ...
        const nomeAba = this.shadowRoot.getElementById('sheet-selector').value;
        const config  = this.ABAS_CONFIG.find(a => a.nome === nomeAba);
        const isSupa  = config?.tipo === 'supabase_mes' || config?.tipo === 'supabase_diario';
        ...
        if (this.currentPage === 1 && filtered.length >= 1) {
            ...
            [1, 0, 2].forEach(idx => {
                ...
                <div class="podium-item ${type}">
                  ...
                  <div class="name" title="${s.nome}"><span class="rank-name-text">${s.nome || 'Sem Nome'}</span>${s.verificado_premium ? this.VERIFICADO_PREMIUM_SVG : (s.verificado ? this.VERIFICADO_SVG : '')}</div>
        ...
        listSlice.forEach((s, i) => {
            ...
            <div class="list-item">
              ...
              <div class="list-name-col">
                <div class="list-name" title="${s.nome}"><span class="rank-name-text">${s.nome || 'Sem Nome'}</span>${s.verificado_premium ? this.VERIFICADO_PREMIUM_SVG : (s.verificado ? this.VERIFICADO_SVG : '')}</div>
