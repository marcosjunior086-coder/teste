...
class DmaiorServicesMenu extends HTMLElement {
  ...
  render() {
    ...
    const SVG_VOTE     = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>`;
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
        ...
        .nav-item-center { --gem-accent:var(--dm-effect-accent,var(--dm-rank-cyan,#00d4d4)); --gem-soft:rgba(0,212,212,.12); --gem-ring:rgba(0,212,212,.20); --gem-glow:rgba(0,212,212,.16); --gem-shadow:rgba(0,212,212,.55); --gem-text-shadow:rgba(0,212,212,.35); position:relative; top:-25px; text-decoration:none; display:flex; flex-direction:column; align-items:center; cursor:pointer; isolation:isolate; }
        ...
        .floating-btn { position:relative; width:75px; height:75px; background:radial-gradient(circle at 50% 48%,rgba(0,0,0,.08) 0%,var(--dm-bg-2,#101113) 52%,var(--dm-bg-3,#000) 100%); border:2px solid var(--gem-accent); border-radius:50%; display:flex; justify-content:center; align-items:center; box-shadow:0 0 0 10px var(--gem-soft),inset 0 0 18px var(--gem-glow); margin-bottom:8px; transition:all .3s ease; overflow:visible; }
        .floating-btn::before { display:none; }
        .floating-btn::after { content:''; position:absolute; inset:-8px; border:1px solid var(--gem-ring); border-radius:50%; opacity:.55; pointer-events:none; }
        .floating-btn svg { position:relative; z-index:1; width:34px; height:34px; color:var(--gem-accent); stroke:var(--gem-accent); fill:none; filter:drop-shadow(0 0 7px var(--gem-shadow)); animation:pulse-gem 2.4s infinite ease-in-out; }
        :host-context(:not([data-theme])) .nav-item-center,
        :host-context([data-theme="dark"]) .nav-item-center,
        :host-context([data-theme="branco"]) .nav-item-center { --gem-accent:#00d4d4; --gem-soft:rgba(0,212,212,.12); --gem-ring:rgba(0,212,212,.20); --gem-glow:rgba(0,212,212,.16); --gem-shadow:rgba(0,212,212,.55); --gem-text-shadow:rgba(0,212,212,.35); }
        :host-context([data-theme="rosa"]) .nav-item-center { --gem-soft:rgba(233,30,140,.12); --gem-ring:rgba(233,30,140,.20); --gem-glow:rgba(233,30,140,.16); --gem-shadow:rgba(233,30,140,.55); --gem-text-shadow:rgba(233,30,140,.35); }
        :host-context([data-theme="laranja"]) .nav-item-center { --gem-soft:rgba(249,115,22,.12); --gem-ring:rgba(249,115,22,.20); --gem-glow:rgba(249,115,22,.16); --gem-shadow:rgba(249,115,22,.55); --gem-text-shadow:rgba(249,115,22,.35); }
        ...
        .nav-item-center span { font-family:var(--dm-font-title,'Rajdhani',sans-serif); font-size:.8rem; font-weight:700; color:var(--gem-accent); text-transform:uppercase; letter-spacing:2px; text-shadow:0 0 7px var(--gem-text-shadow); }
        ...
        .dp-banner { margin:0 16px 12px; border-radius:26px; overflow:hidden; position:relative; background:var(--dm-grad-card-alt); border:1px solid var(--dm-effect-35,var(--dm-rank-cyan-35,rgba(0,212,212,.35))); box-shadow:0 8px 24px var(--dm-shadow-50),0 0 20px var(--dm-effect-glow,var(--dm-rank-glow,rgba(59,130,246,.28))); text-decoration:none; display:flex; align-items:center; padding:14px 16px; gap:14px; }
        .dp-banner::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,transparent,var(--dm-effect-blue,var(--dm-rank-blue,#3b82f6)),var(--dm-effect-accent,var(--dm-rank-cyan,#00d4d4)),transparent); }
      ...
      <div class="container" style="flex-direction:column">
        ...
        <div class="mobile-layout" style="display:${layout === 'dinamico' ? 'none' : 'flex'}">
          ...
          <div class="menu-panel">
            ...
            <a href="votacao.html" class="nav-item item-votacao">
              ${SVG_VOTE}
              <span data-i18n="homeVote">Votação</span>
            </a>
          </div>
        </div>
        ...
        <div class="dinamico-layout">
          ...
          <div class="dp-grid">
            ...
            <a href="votacao.html" class="dp-item dp-teal">
              <div class="dp-item-icon">${SVG_VOTE}</div>
              <div class="dp-item-text">
                <div class="dp-item-name" data-i18n="homeVote">Votação</div>
                <div class="dp-item-label" data-i18n="homeVoteNow">Participe agora</div>
              </div>
              <div class="dp-item-arrow">${SVG_ARROW}</div>
            </a>
